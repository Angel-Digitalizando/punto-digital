...
## Pendiente documentado — Escalado tipográfico en contenido dinámico

**Estado:** No implementado. Documentado para próxima iteración.

**Problema detectado:**
El sistema `--fs-scale` funciona correctamente en componentes estáticos (menú, drawer, header, footer). Sin embargo, parte del contenido generado dinámicamente desde `ui.js` y `tutoriales.js` usa ocasionalmente tamaños en `rem` hardcodeados que no consumen `--fs-scale`.

**Causa probable:**
Los pasos de los tutoriales se renderizan via `innerHTML` con clases como `.paso-tutorial p` que sí usan `var(--fs-lg)`, pero algunos renderizadores secundarios (nav anterior/siguiente, barra de acciones) aún tienen `font-size` en `rem` directo.

**Plan para próxima iteración:**
1. Auditar todos los `font-size` en `ui.js` y reemplazar por variables `--fs-*`
2. Verificar `.btn-nav`, `.nav-tutorial-contador`, `.acciones-tutorial` 
3. Confirmar que el cambio de `data-fs` en `<html>` recalcula correctamente los rem heredados

**No implementado en esta fase para no afectar contexto de sesión.**
